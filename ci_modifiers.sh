...
for tool_pair in "docker:docker completion zsh" "kubectl:kubectl completion zsh" "helm:helm completion zsh" "orb:orb completion zsh" "orbctl:orbctl completion zsh"; do
    ...
    if command -v "$tool" > /dev/null 2>&1; then
        ...
        if [[ "$tool" == "docker" ]]; then
            ...
            # Approach 3: Create a working minimal completion file as ultimate fallback
            if [[ "$docker_completion_created" == "false" ]] && command -v docker > /dev/null 2>&1 && docker --version > /dev/null 2>&1; then
                echo "[CI] Creating comprehensive minimal Docker completion file..."
