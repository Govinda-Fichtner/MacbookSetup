...
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq empty"
The status should be success
End

It 'validates all servers have docker command'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers[].command'"
The status should be success
The output should include "docker"
End

It 'validates all servers have args array'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers[].args | type'"
The status should be success
The output should include "array"
End

It 'validates env-file arguments are properly formatted'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers.github.args[]' | grep env-file"
The status should be success
The output should include "--env-file"
End

It 'validates filesystem server has volume mounts'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers.filesystem.args[]' | grep volume"
The status should be success
The output should include "--volume"
End

It 'validates privileged servers have appropriate flags'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers.docker.args | join(\" \")'"
The status should be success
The output should include "/var/run/docker.sock"
End

It 'validates kubernetes server has network host'
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers.kubernetes.args[]' | grep network"
The status should be success
...
When run sh -c "./mcp_manager.sh config 2>/dev/null  | jq '.mcpServers[].args[]' | grep -E '^\"--[a-z-]+='"
