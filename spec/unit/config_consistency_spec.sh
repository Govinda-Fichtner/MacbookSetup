...
config_count=$(./mcp_manager.sh config 2> /dev/null | jq '.mcpServers | keys | length')
...
config1=$(./mcp_manager.sh config 2> /dev/null | jq -S .)
config2=$(./mcp_manager.sh config 2> /dev/null | jq -S .)
...
When run sh -c "./mcp_manager.sh config 2>/dev/null | jq '.mcpServers[].name // \"MISSING\"' | grep -v MISSING | wc -l"
The status should be success
The output should not equal "0"
End

It 'no server configurations are null'
When run sh -c "./mcp_manager.sh config 2>/dev/null | jq '.mcpServers[] | select(. == null)'"
The status should be success
